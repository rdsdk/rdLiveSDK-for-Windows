...
void RDLiveSdkDemo::on_menuScene_aboutToShow()
{
	...
	for ( int i = 0; i < Render_GetSceneCount(); ++i )
	{
		...
		if ( szName.isEmpty() )
		{
			szName	= QString::fromUtf16( L"场景 %1" ).arg( i + 1 );
		}
	...
	actScene	= ui.menuDeleteScene->addAction( QString::fromUtf16( L"删除所有空场景" ) );
	actScene->setData( -1 );
...
void RDLiveSdkDemo::on_menuRenameScene_triggered ( QAction * action )
{
	...
	if ( szName.isEmpty() )
	{
		szName	= QString::fromUtf16( L"场景 %1" ).arg( Render_GetSceneIndex( hScene ) + 1 );
	}
	szName	= QInputDialog::getText( this, QString::fromUtf16( L"场景重命名" ), QString::fromUtf16( L"新的名称：" ),
		QLineEdit::Normal, szName, &bOk );
	if ( bOk )
	{
		Scene_SetName( hScene, szName.utf16() );
	}
}
...
{
	bool	bOk		= false;
	QString	szName	= QInputDialog::getText( this, QString::fromUtf16( L"添加场景" ), QString::fromUtf16( L"场景名称：" ),
...
void RDLiveSdkDemo::UpdatePreviewConfig( IGlRender_SPreviewLayout::EReposWnd eMode )
{
	...
	m_pLabFScale->setText( QString::fromUtf16(L" 预览：%1% ").arg( int( m_sPerviewLayout.fScale * 100.0f + 0.5f ) ) );
	//把当前的预览布局写入 XML 对象
...
void RDLiveSdkDemo::on_menuAddScreen_aboutToShow()
{
	...
	szItem	= QString::fromUtf16( L"整个桌面 [%1x%2]" ).arg( rtScreen.width() ).arg( rtScreen.height() );
	...
	if ( iScreenCount > 1 )
	{
		for ( int i = 0; i < iScreenCount; ++i )
		{
			...
			rtScreen	= QRect( monit.rcMonitor.left, monit.rcMonitor.top, 
				monit.rcMonitor.right - monit.rcMonitor.left, monit.rcMonitor.bottom - monit.rcMonitor.top );
			szItem	= QString::fromUtf16( L"屏幕 %1 [%2x%3]" ).arg(i).arg( rtScreen.width() ).arg( rtScreen.height() );
	...
	while( hWnd )
	{
		...
		if ( bAddWindow )
		{
			...
			{
				szItem	= fm.elidedText( QString::fromUtf16( szWindow ), Qt::ElideRight, 200 );
...
void RDLiveSdkDemo::on_menuAddCamera_aboutToShow()
{
	...
	if ( iCameraCount )
	{
		...
		{
			QAction*	actCamera	= ui.menuAddCamera->addAction( QString::fromUtf16( Camera_GetFriendlyName( i ) ) );
			actCamera->setData( QString::fromUtf16( Camera_GetDisplayName( i ) ) );
		}
	}
	else
	{
...
void RDLiveSdkDemo::on_menuAddCamera_triggered ( QAction * action )
{
	...
	if ( hChip )
	{
		Chip_SetRectPercent( hChip, 0.0f, 0.0f, 1.0f, 1.0f, eKeepAspectRatio );
		Chip_SetVisible( hChip, TRUE );
		if ( Chip_Open( hChip, L"Camera", action->data().toString().utf16() ) )
		{
		}
	}
}
...
void RDLiveSdkDemo::on_actiAddPicture_triggered( bool checked )
{
	...
	if ( hChip )
	{
		Chip_SetRectPercent( hChip, 0.0f, 0.0f, 1.0f, 1.0f, eKeepAspectRatio );
		Chip_SetVisible( hChip, TRUE );
		if ( Chip_Open( hChip, L"Picture", szFileName.utf16() ) )
		{
		}
	}
